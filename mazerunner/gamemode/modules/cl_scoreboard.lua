	...
	for _, v in pairs({0, 1, 2}) do
		...
		for _, ply in pairs(team.GetPlayers(v)) do
			...
			pnl.Paint = function(s, w, h)
				...
				local lives = ply:GetLives()
				local lifeText = " Lives"
				if lives == 1 then lifeText = " Life" end
				draw.SimpleText(lives .. lifeText, "SBPlayer", w*29/31, h/2, Color(0, 0, 0, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				draw.SimpleText(ply:GetMRClass(), "SBPlayer", w/100, h/2, Color(0, 0, 0, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			end
